...
func handleGroupByFuncExprDateHisto(funcExpr *sqlparser.FuncExpr) (msi, error) {
	...
	for _, expr := range funcExpr.Exprs {
		...
		case *sqlparser.AliasedExpr:
			...
			if !ok {
				return nil, errors.New("sqlToES: unsupported expression in date_histogram")
			}
...
func handleGroupByFuncExprRange(funcExpr *sqlparser.FuncExpr) (msi, error) {
	if len(funcExpr.Exprs) < 3 {
		return nil, errors.New("sqlToES: length of function range params must be > 3")
	}
...
func handleGroupByFuncExprDateRange(funcExpr *sqlparser.FuncExpr) (msi, error) {
	...
	for _, expr := range funcExpr.Exprs {
		...
		if !ok {
			return nil, errors.New("sqlToES: unsupported star expression in function date_range")
		}
		...
		case *sqlparser.ComparisonExpr:
			...
			case "format":
				format = equalVal
			default:
				return nil, errors.New("sqlToES: unsupported column name " + colName)
			}
		case *sqlparser.SQLVal:
			skippedString := strings.Trim(sqlparser.String(item), "`")
			rangeList = append(rangeList, skippedString)
		default:
			return nil, errors.New("sqlToES: unsupported expression " + sqlparser.String(expr))
		}
	}

	if len(field) == 0 {
		return nil, errors.New("sqlToES: lack field of date_range")
	}
...
func handleGroupByFuncExpr(funcExpr *sqlparser.FuncExpr, child msi) (msi, error) {
	...
	default:
		return nil, errors.New("sqlToES: unsupported group by functions" + sqlparser.String(funcExpr))
	}
