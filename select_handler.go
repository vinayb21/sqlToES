...
func handleSelect(sel *sqlparser.Select) (dsl string, esType string, err error) {
	...
	//Handle from
	if len(sel.From) != 1 {
		return "", "", errors.New("sqlToES: multiple from currently not supported")
	}
...
func buildNestedFuncStrValue(nestedFunc *sqlparser.FuncExpr) (string, error) {
	return "", errors.New("sqlToES: unsupported function" + nestedFunc.Name.String())
}
...
func buildComparisonExprRightStr(expr sqlparser.Expr) (string, bool, error) {
	...
	case *sqlparser.GroupConcatExpr:
		return "", missingCheck, errors.New("sqlToES: group_concat not supported")
	...
	case *sqlparser.ColName:
		...
		return "", missingCheck, errors.New("sqlToES: column name on the right side of compare operator is not supported")
...
func handleSelectWhereComparisonExpr(expr *sqlparser.Expr, topLevel bool, parent *sqlparser.Expr) (string, error) {
	...
	if !ok {
		return "", errors.New("sqlToES: invalid comparison expression, the left must be a column name")
	}
...
func handleSelectWhere(expr *sqlparser.Expr, topLevel bool, parent *sqlparser.Expr) (string, error) {
	if expr == nil {
		return "", errors.New("sqlToES: error expression cannot be nil here")
	}
	...
	case *sqlparser.IsExpr:
		return "", errors.New("sqlToES: is expression currently not supported")
	case *sqlparser.RangeCond:
		...
		if !ok {
			return "", errors.New("sqlToES: range column name missing")
		}
	...
	case *sqlparser.NotExpr:
		return "", errors.New("sqlToES: not expression currently not supported")
	case *sqlparser.FuncExpr:
		switch e.Name.Lowered() {
		case "multi_match":
			params := e.Exprs
			if len(params) > 3 || len(params) < 2 {
				return "", errors.New("sqlToES: the multi_match must have 2 or 3 params, (query, fields and type) or (query, fields)")
			}
			...
			for i := 0; i < len(params); i++ {
				...
				if len(kv) != 2 {
					return "", errors.New("sqlToES: the param should be query = xxx, field = yyy, type = zzz")
				}
